...
type LecturerProjectSubmissionsClientProps = {
  user: {
    id: string;
    email: string;
    role: string;
...
export default function LecturerProjectSubmissionsClient({
  user,
  courseSlug,
  projectId,
}: LecturerProjectSubmissionsClientProps) {
  ...
  const basePath = `/api/lecturers/${user.id}/courses/${courseSlug}/projects/${projectId}/submissions`;
    ...
    async function load() {
      try {
        ...
        if (!res.ok || !json.success) {
          throw new Error(json.message || "Gagal memuat Karya Mahasiswa");
        }
      ...
      {loading ? (
        ...
        <>
          <section className="rounded-3xl bg-teal-600 p-6 text-white sm:p-8">
            <p className="text-sm text-teal-50">{courseTitle}</p>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              <StatCard label="Total Karya Mahasiswa" value={stats.total} />
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-6">
      ...
      <div className="mt-4 grid gap-3">
        <ProjectBlock label="Judul" value={submission.title} />
        <ProjectBlock label="Ringkasan" value={submission.summary} />
        {submission.artifactUrl ? (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
            ...
            <a
              ...
              className="mt-2 inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-base font-medium text-teal-700 transition hover:bg-slate-50"
            >
              <ExternalLink size={16} className="shrink-0" aria-hidden />
              <span className="break-all">{submission.artifactUrl}</span>
            </a>
          </div>
        ) : null}
        <ProjectBlock label="Refleksi" value={submission.reflection} />
      </div>
        ...
        <div className="mt-5 grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-base font-medium text-slate-700 transition hover:bg-slate-50 disabled:opacity-60"
            >
