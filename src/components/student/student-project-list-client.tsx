...
type StudentProjectListClientProps = {
  user: {
    id: string;
    email: string;
    role: string;
...
export default function StudentProjectListClient({
  user,
  courseSlug,
}: StudentProjectListClientProps) {
    ...
    async function load() {
      try {
        ...
        const res = await fetch(
          `/api/students/${user.id}/courses/${courseSlug}/projects`,
          { cache: "no-store" },
        );
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-3xl bg-teal-600 px-6 py-7 text-white sm:px-8">
        ...
        <div className="mt-5 inline-flex max-w-full items-center gap-2 rounded-full bg-white/15 px-4 py-2 text-base">
          <Rocket size={18} className="shrink-0" aria-hidden="true" />
          <span className="truncate">{courseTitle || courseSlug}</span>
        </div>

        <h1 className="mt-4 text-2xl font-bold sm:text-3xl">Project Aksi</h1>
        <p className="mt-2 max-w-3xl text-lg leading-relaxed text-teal-50">
          Wujudkan pembelajaran menjadi aksi nyata: rancang, kerjakan, lalu
          kumpulkan project beserta refleksi Anda.
        </p>
      </section>
      ...
      <section className="space-y-4">
        {loading ? (
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 text-base text-slate-600">
            Belum ada project pada mata kuliah ini.
          </div>
        ) : (
          projects.map((project) => {
            const status = project.submission?.status;
            ...
            return (
              ...
              >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-sm text-slate-500">
                      ...
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1">
                        ...
                        Tenggat: {formatDate(project.dueAt)}
                      </span>
                      ...
                      project.submission?.score !== undefined ? (
                        <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-100 px-3 py-1 text-emerald-700">
                          ...
                          Nilai: {project.submission.score}
                        </span>
                      ) : null}
                    </div>
                  </div>
                ...
                <div className="mt-5">
                  <Link
                    href={`/student/courses/${courseSlug}/projects/${project.id}`}
